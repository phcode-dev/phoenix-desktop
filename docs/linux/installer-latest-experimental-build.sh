...
# Check if jq is installed. This is only used in dev builds and not actual prod builds as jq
# is not a default in most systems
if ! command -v jq &> /dev/null
then
    echo "jq is not installed. This check is only present in dev builds"
    # Provide installation command based on the package manager.
    # Adjust the command according to the target Linux distribution's package manager.
    echo "To install jq, run one of the following commands based on your Linux distribution:"

    # Suggest installation commands for different package managers
    ...
    read -n 1 -s -r -p "Press any key to continue after installing jq"
    echo
fi
...
# Fetch the list of releases, assuming the first prerelease in the list is the latest, get the prerelase api url
API_URL=$(wget -qO- "$RELEASES_URL" | jq -r '[.[] | select(.prerelease == true)][0].url')
echo "$API_URL"
...
DESKTOP_APP_NAME="Phoenix Code Experimental Build"
...
downloadLatestReleaseInfo() {
  ...
  if [ -f "$release_info_file" ]; then
    ...
    grep -Po '"tag_name": "dev-app-v\K[\d.]+(?=")' "$release_info_file"
    return
  fi
...
downloadAndInstall(){
  ...
  if [ -f "$TMP_DIR/phoenix-code/phoenix-code-experimental-build" ]; then
      echo "The file $TMP_DIR/phoenix-code/phoenix-code-experimental-build exist. Dev build detected."
      mv "$TMP_DIR/phoenix-code/phoenix-code-experimental-build" "$TMP_DIR/phoenix-code/phoenix-code"
  fi
